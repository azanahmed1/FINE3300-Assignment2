...
file_path = os.path.dirname(os.path.abspath(__file__))  # Get the current folder path
...
for file in files:
    df = pd.read_csv(file)
    province = os.path.basename(file).split(".")[0]  # Extract province name from file name
    df['Jurisdiction'] = province  # Add province column
    df_list.append(df)
...
cpi_data = cpi_data.dropna(subset=['CPI'])  # Drop NaN CPI values
cpi_data = cpi_data[cpi_data['CPI'] > 0]  # Remove zero values (to avoid division errors)
cpi_data['Monthly Change'] = cpi_data.groupby(['Jurisdiction', 'Item'])['CPI'].pct_change(fill_method=None) * 100
cpi_data['Monthly Change'] = cpi_data['Monthly Change'].clip(-100, 100)  # Remove extreme outliers
